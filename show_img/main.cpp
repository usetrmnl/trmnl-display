...
const char *szAdapters[] = {"sdl", "framebuffer", "pimoroni", "waveshare_2", "waveshare_2_opi_rv2", NULL};
...
#ifndef __SDL__
    if (iAdapter > ADAPTER_FRAMEBUFFER && (jpg.getWidth() > bbep.width() || jpg.getHeight() > bbep.height())) {
...
int DecodeImage(const char *fname) {
    ...
    // Easiest to read the entire image file into memory (Linux has plenty of RAM)
...
char szFile[256];
    ...
    // These will override the settings from the JSON file
    //
    if (argc > 1) {
...
#ifdef __SDL__
    iAdapter = ADAPTER_SDL; // only 1 choice possible in SDL mode
#endif // __SDL__

    if (iAdapter == ADAPTER_SDL || iAdapter == ADAPTER_FRAMEBUFFER) { // for sdl & framebuffer, some parameters don't matter
    ...
    if (iAdapter == ADAPTER_SDL || iAdapter == ADAPTER_FRAMEBUFFER) { // sdl & framebuffer
#ifdef __SDL__
        // See if we're the first instance to run, if not, pass the filename and leave
        channel = nj_ipc_channel_open("show_img", 256);
        if (channel.status == SUCCESS) { // we're not the first instance
            printf("We're not the first instance; exiting...\n");
            nj_ipc_channel_write(&channel, (void *)szFile, 256);
            return 0; // tell the other instance to show it
        }
        // Create a channel to listen for the next instance
        printf("Creating ipc channel...\n");
        channel = nj_ipc_channel_create("show_img", 256);
        iMode = 0; // write a 0 length into the channel
        nj_ipc_channel_write(&channel, (void *)&iMode, 1);
#endif // __SDL__
    } else {
#ifndef __SDL__
    ...
    if (!DecodeImage(szFile)) {
        return -1; // decoding failed, exit program
    }
    
#ifdef SHOW_DETAILS
    printf("image specs: %d x %d, %d-bpp\n", iWidth, iHeight, iBpp);
#endif
    if (iAdapter == ADAPTER_SDL) {
        win = SDL_CreateWindow("TRMNL", 100, 100, 800, 480, SDL_WINDOW_SHOWN);
        if (win == nullptr) {
        ...
        while (!bQuit) {
            ...
            nj_ipc_channel_read(&channel, (void *)szTemp, 256);
            if (szTemp[0]) { // new name waiting
                printf("new file: %s\n", szTemp);
                if (DecodeImage(szTemp)) {
                    ShowSDLImage(); // show the new image on the existing window
                }
                szTemp[0] = 0;
                nj_ipc_channel_write(&channel, (void *)szTemp, 256); // clear the flag
            }
            SDL_Delay(100);
        } // while SDL window displayed

        // Clean up
        nj_ipc_channel_free(&channel); // free ipc resources
        SDL_FreeSurface(canvas);
        SDL_FreeSurface(winSurface);
        SDL_DestroyWindow(win);
        SDL_Quit();
    } else if (iAdapter == ADAPTER_FRAMEBUFFER) { // framebuffer
        UpdateFramebuffer();
    } else { // e-paper
#ifndef __SDL__
...
#if defined( SHOW_DETAILS ) && !defined( __SDL__ )
